...
import { useGetFollowerQuery, } from '@/features/follow/followApi';
...
const FollowList = () => {
    ...
    const {
        ...
        isFetching,
        isError,
        refetch } = useGetFollowerQuery({ userId, page }, { refetchOnMountOrArgChange: true });


    const hasNextPage = followerList?.hasNextPage;
    const nextPage = followerList?.nextPage;
    ...
    const renderHeader = () => {
        return (
            <View className="h-20 w-[90%] mt-[10%] px-2 self-center">
                <Text className="font-pextrabold text-3xl" > Follower</Text>
    ...
    const renderFooter = () => (
        <View style={{ marginVertical: 16, alignItems: 'center' }}>
            {isFetching && !refreshing ? (
                ...
                <Text style={{ color: '#888' }} className="font-plight">No more Followers</Text>
            )}
        </View>
    );
